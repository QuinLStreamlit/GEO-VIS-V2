...
def plot_SPT_vs_depth_granular(
    ...
    """
    Create professional SPT vs Depth scatter plot for granular soils with density classification indicators.
    
    This function creates scatter plots showing Standard Penetration Test (SPT) N values versus 
    depth with granular soil density classification boundaries and indicators. 
    Designed for geotechnical analysis following standard granular soil density classifications.
    ...
    === Plot Appearance ===
    title : str, optional
        Custom plot title. If None, uses default "SPT vs Depth (Granular Soils)".
        Example: "Borehole SPT Analysis - Granular Layers"
        
    title_suffix : str, optional
        Text to append to default title.
        Example: ": Phase 2 Results" → "SPT vs Depth (Granular Soils): Phase 2 Results"
    ...
    xlim : tuple of (float, float), optional
        ...
        Example: xlim=(0, 80) sets SPT axis from 0 to 80 blows
    ...
    show_strength_indicators : bool, default True
        Whether to show SPT density classification labels between boundary lines.
        Displays: VL, L, MD, D, VD labels positioned between respective boundaries.
        
    show_strength_boundaries : bool, default True
        Whether to show vertical lines at SPT classification boundaries.
        Lines at: 4, 10, 30, 50 SPT N (standard granular soil boundaries)
        
    show_strength_boundary_ticks : bool, default True
        Whether to show strength boundary values as x-axis tick labels.
        When True, displays ticks at boundary values (0, 4, 10, 30, 50).
    ...
    output_filepath : str, optional
        ...
        Example: 'results/spt_depth_granular.png'
    ...
    --------
    **Basic usage:**
    >>> plot_SPT_vs_depth_granular(df)
    
    **With custom columns:**
    >>> plot_SPT_vs_depth_granular(df, depth_col='Depth_m', spt_col='N_value')
    
    **With geological categories:**
    >>> plot_SPT_vs_depth_granular(df, category_col='Material', 
    ...                           title="Site Investigation - Granular Layers")
    
    **Custom axis limits and styling:**
    >>> plot_SPT_vs_depth_granular(df, xlim=(0, 80), ylim=(0, 30),
    ...
    **Custom strength indicator positioning:**
    >>> plot_SPT_vs_depth_granular(df, strength_indicator_position=0.1)  # Near top
    >>> plot_SPT_vs_depth_granular(df, strength_indicator_position=0.8)  # Near bottom
    
    **Save without displaying:**
    >>> plot_SPT_vs_depth_granular(df, show_plot=False,
    ...                           output_filepath='spt_granular_analysis.png')
    """
    # === Input Validation ===
    ...
    if xlim is not None:
        ...
        # Default SPT range for granular soils
        ax.set_xlim(0, 50)
    ...
    if show_strength_indicators:
        ...
        for min_val, max_val, label in strength_ranges:
            if max_val == float('inf'):
                # For the last category (VD), position within plot bounds
    ...
    print("--- SPT vs Depth (Granular) plotting finished ---")